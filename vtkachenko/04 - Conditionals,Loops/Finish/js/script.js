...
let phoneCheck = function(phoneNumber){
  if(phoneNumber.length < 9 || phoneNumber.length > 11 ){
    alert('Phone < 9 and >11');
  }
};
...
// * Your code Start *
let checkUser = function(users){
  let userName = prompt('Enter user name');
  for(var key in users){
    (userName == users[key].name) ? checkPass(users[key]) : checkUser(users);
  }
}
let checkPass = function(user){
...
const summetNumber = Number(prompt('Please enter number', 0));

// * Your code Start *
let numberSum = function(summetNumber){
  return(summetNumber != 1) ? summetNumber + numberSum(summetNumber-1) : 1;
};
console.log(numberSum(summetNumber));
// * Your code End *
...
const drawFraction = function(){
  const board = document.querySelector('.board');
  let fraction;
  let switchBlackWhite = true;
  const size = 8;
  for(let i = 0;i < size;i++){
    for(let j = 0;j < size;j++){
      if(j==0) switchBlackWhite = !switchBlackWhite; 
      fraction = document.createElement('div');
      switchBlackWhite ? fraction.className = 'black':fraction.className = 'white';
      board.appendChild(fraction);
      switchBlackWhite = !switchBlackWhite;
    }
  }
};
drawFraction();
